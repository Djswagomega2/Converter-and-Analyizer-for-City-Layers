...
def load_workbook(filename):
    ...
    print(f"Loading: {filename}") #prints the name of the sheet we are loading
    sheets = pd.read_excel(filename,sheet_name=None)#puts that sheet into pandas

    #If we do not find a xml key sheet
    if xml_key_sheet not in sheets:
        raise ValueError(f"Workbook must contain a '{xml_key_sheet}' sheet") #we print an error

    #separate XML configuration sheet from the rest of the data
    xml_key = sheets.pop(xml_key_sheet)

    return sheets,xml_key

def clean_xml_key(xml_key):
...
def create_value_element(parent_element,row,config):
    ...
    value = get_value(row,source_column) #we get the value from the row and source column

    _,unit = parse_header(source_column)  #Parse the unit from Excel header
...
def create_repeating_values(parent_element,row,config,children=None):
    ...
    _,unit = parse_header(source_column)
    attributes = {}
...
def find_related_rows(data,source_sheet,relationship,parent_value):
...
def build_node(parent_element,node,row,data):
    ...
    match node_type: #we switch on the node type
        case "value":
            ...
            if row is not None and source_column in row.index:
                create_value_element(parent_element,row,config)
                ...
                if len(source_data) > 0:
                    ...
                    create_value_element(parent_element,source_row,config)
            else:
        ...
        case "list":
            if row is None:
                return
            create_repeating_values(parent_element,row,config,node["children"]) #if the node type is a list, we create the new xml elements for the repeating values
            return
        case "container":
            ...
            for child in node["children"]:
                build_node(element,child,row,data)
        case "repeating":
            ...
            if row is None:
                for _, child_row in source_data.iterrows():
                    ...
                    for child in node["children"]:
                        build_node(element,child,child_row,data)
                return
            ...
            if relationship and relationship in row.index:
                parent_value = get_value(row,relationship) #we get the parent value from the row and relationship

                if parent_value is None:
                    return

                related_rows = find_related_rows(data,source_sheet,relationship,parent_value) #we find the related rows in the source sheet based on the relationship and parent value
...
def build_xml(data, xml_key):
    ...
    """
    tree = build_key_tree(xml_key) #build the actual tree once, so reused tag names don't get merged together
    ...
    for child in tree["children"]:
        ...
        if node_type == "repeating":
            ...
            dataframe = data[source_sheet]

            # Build one XML object for every row
            for _, row in dataframe.iterrows():
                build_node(root_element,child,row,data)
        elif node_type == "container":
            build_node(root_element,child,None,data)
        else:
            raise ValueError(f"Unsupported root node type "f"'{node_type}' for tag '{tag}'")
    return root_element

def save_xml(root_element,filename):
...
def main():
    ...
    print("Starting Excel → XML Converter")

    data, xml_key = load_workbook(input_file) #we load the workbook, and extract the data we want to convert and the xml key and set that to their own variables 

    # Clean XML key
    xml_key = clean_xml_key(xml_key)

    # Clean column names in all data sheets
    for sheet_name, dataframe in data.items():
        dataframe.columns = (dataframe.columns.astype(str).str.strip())

    # Build XML
    root_element = build_xml(data,xml_key)

    # Save XML
    save_xml(root_element,output_file)

    print("Excel → XML conversion complete!")
